...
class Menu extends BaseActiveRecord
{
    ...
    public static function getBreadcrumbs($rule = 'index/index'){
        ...
        $current = static::find()->select('id')->where(['like', 'url', $rule])->asArray()->one();
     ...
     * ---------------------------------------
     */
    public static function getMenus($rule = 'site/index'){
        ...
        foreach ($groups as $k => $g) {
            $menuList = static::find()
                ->where(['hide'=>0, 'group'=>$g])
                ->orderBy('sort ASC')->asArray()->all();
            /* 判断二级菜单权限 */
            if ($menuList && is_array($menuList)) {
                foreach ($menuList as $skey => $check_menu) {
                    if ($nav[0]['id'] == $check_menu['id']) {
                    /* 设置当前菜单的一级菜单高亮 */
                        $menus[$k]['class']='active';
                    }
                    if ($check_menu['url'] == $rule){
                        $menuList[$skey]['class']='active';
                    }
